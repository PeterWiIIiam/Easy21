...
def perform_sequence(N, Q, total_trials, init_state=None, init_action=None):
    ...
    while state != None:
        epsilon = 1
        visitedTimes = sum(N[state["dealer_sum"], state["player_sum"], :])
        # epsilon = 1/(1 + np.log(visitedTimes))
        epsilon = 1 - visitedTimes / total_trials
        print(epsilon)
            ...
            for player_card_value in range(2, 22):
                for action in [0, 1]:
                    ...
                    state_sequence, award_sequence, action_sequence = \
                        perform_sequence(N, Q, total_trials=total_trials, init_state=state, init_action=action)
